...
opts.Add(
    PathVariable(
        "target_path", "The path where the lib is installed.", "project/gdnative/"
    )
)
opts.Add(
    PathVariable(
...
if platform == "osx":
    ...
    if env["target"] in ("debug", "d"):
        env.Append(CCFLAGS=["-g", "-O2", "-arch", "arm64", "-std=c++14"])
        env.Append(LINKFLAGS=["-arch", "arm64"])
    else:
        env.Append(CCFLAGS=["-g", "-O3", "-arch", "arm64", "-std=c++14"])
        env.Append(LINKFLAGS=["-arch", "arm64"])
elif platform == "linux":
    if env["target"] in ("debug", "d"):
...
env.Append(
    LIBS=[
        env.File(
            os.path.join(
                "godot-cpp/bin",
                "libgodot-cpp.%s.%s.64%s" % (platform, env["target"], env["LIBSUFFIX"]),
            )
        )
    ]
)
...
library = env.SharedLibrary(
    target=env["target_path"] + "/" + platform + "/" + env["target_name"],
    source=sources,
)
Default(library)
